...
const questionsBatch3 = [
    ...
    {
        id: 202,
        question: "During a penetration test against a network defended by a signature-based Intrusion Detection System (IDS), the tester initiates a port scan but notices that traditional scanning methods are being flagged and blocked. To evade detection, the tester modifies their approach: they split the TCP headers into multiple smaller IP segments before sending them across the network. Once these segments reach the target, the destination host reassembles them and processes the scan as a normal SYN request.\n\nWhich evasion technique has the tester most likely employed to avoid triggering IDS alerts?",
        options: [
    ...
    {
        id: 203,
        question: "During a security assessment of an organization's internal network, a penetration tester discovers that UDP port 123 is open on a critical host, indicating that the Network Time Protocol (NTP) service is active. The tester decides to enumerate the list of NTP peers and determine their synchronization status, offset, and stratum levels.\n\nWhich command should the tester use to retrieve this specific information?",
        options: [
            "ntpq",
            "ntpdc",
            "ntpdate",
    ...
    {
        id: 215,
        question: "A penetration tester is tasked with uncovering historical content from a company's website, including previously exposed login portals or sensitive internal pages. Direct interaction with the live site is prohibited due to strict monitoring policies.\n\nWhich approach would most effectively support this passive information-gathering objective?",
        options: [
    ...
    {
        id: 218,
        question: "While conducting a covert penetration test on a UNIX-based infrastructure, the tester decides to bypass intrusion detection systems by sending specially crafted TCP packets with an unusual set of flags enabled. During the scan, the tester notices that when certain ports are probed, there is no response from the target, but for others, a TCP RST packet is received.\n\nBased on these observations, which scanning technique is most likely being used?",
        options: [
    ...
    {
        id: 226,
        question: "During a cybersecurity awareness drill, an ethical hacker is assigned the task of mapping a large multinational enterprise's external attack surfaces. Due to strict rules of engagement, the hacker must avoid any active scans or probes that could alert the organization's security monitoring systems. The goal is to uncover as many publicly exposed subdomains as possible.\n\nWhich method should the ethical hacker use to passively enumerate the organization's subdomains?",
        options: [
    ...
    {
        id: 227,
        question: "During a stealth assessment of a corporate internal network, an attacker gains access to a subnet where host communications are switched. While passively sniffing traffic, the attacker observes that the target system intermittently delays its response to ARP requests. The attacker crafts and injects a fake ARP reply that maps the target's IP address to their own MAC address and sends it before the legitimate reply arrives.\n\nWhat type of sniffing attack is the attacker performing?",
        options: [
    ...
    {
        id: 229,
        question: "During a security assessment, a consultant investigates how the application handles requests from authenticated users. They discover that once a user logs in, the application does not verify the origin of subsequent requests. The consultant creates a web page containing a malicious form that submits a funds transfer request to the application. A logged-in user fills out the form and submits it, and the application processes the request successfully without any reauthentication.\n\nWhich session hijacking technique is being used in this scenario?",
        options: [
    ...
    },
    {
        ...
        options: [
            ...
            "Inject a script like <script>alert('XSS')</script> to test for Cross-Site Scripting (XSS)",
    ...
    {
        id: 232,
        question: "During a black-box internal penetration test, a security analyst is tasked with identifying potentially exploitable services running on an SNMP-enabled Linux server. The target organization uses SNMPv2, and the default community string 'public' has not been changed. To gather service-related intelligence, the analyst decides to enumerate all running processes on the host.\n\nWhich Nmap command would most effectively retrieve the required information?",
        options: [
    ...
    {
        id: 234,
        question: "During a red team engagement simulating a targeted attack on a smart office environment, an ethical hacker identifies a thermostat used for regulating temperature. While analyzing its firmware management process, the tester discovers that the device accepts older versions without verifying their integrity or authenticity. The attacker successfully loads a previously deprecated firmware that contains known vulnerabilities.\n\nWhich IoT security issue is most accurately demonstrated in this scenario?",
        options: [
    ...
    {
        id: 235,
        question: "During a security assessment of a company's internal web application, a penetration tester examines how the application processes input values passed in the web address query string. By altering this input to include sequences like ../../../../etc/passwd, the tester successfully navigates beyond the intended storage location and accesses restricted files.\n\nWhich attack method does this scenario illustrate?",
        options: [
    ...
    {
        id: 240,
        question: "An AWS security operations team receives an alert regarding abnormal outbound traffic from an EC2 instance. Upon investigation, it is discovered that a malicious executable was silently installed on the instance, which modifies the sync configuration of the Dropbox client to use the attacker's access token. This enables the compromised EC2 instance to automatically sync selected data folders with the attacker's Dropbox storage.\n\nWhat type of attack has likely occurred?",
        options: [
    ...
    {
        id: 242,
        question: "During an internal red team engagement, an operator discovers that TCP port 389 is open on a target system identified as a domain controller. To assess the extent of LDAP exposure, the operator runs the command ldapsearch -h <target> -x -s base namingcontexts and receives a response revealing the base distinguished name (DN: DC=internal,DC=corp).\n\nWhat is the most logical next action?",
        options: [
    ...
    {
        id: 243,
        question: "During a security assessment of a metropolitan public transportation terminal, a penetration tester examines a network-connected IoT surveillance camera system. Upon analyzing the camera's network traffic, the tester observes that it uses an outdated encryption protocol (SSLv2) to transmit video data. The tester uses a network packet sniffer to intercept this traffic and easily decrypts the stream.\n\nWhat IoT vulnerability is most likely being exploited in this scenario?",
        options: [
    ...
    {
        id: 247,
        question: "A Certified Ethical Hacker (CEH) is auditing a company's web server that employs virtual hosting. The server hosts multiple domains and uses a web proxy to maintain anonymity. The CEH discovers that the server's document root directory is named 'certroot' and is stored in the directory /admin/web.\n\nGiven this scenario, which of the following actions would most likely increase the security of the web server?",
        options: [
    ...
    {
        id: 250,
        question: "In the bustling digital marketplace of Miami's tech corridor, ethical hacker Sofia Alvarez probes the virtual defenses of RetailRush, a US-based online retailer. Tasked with exposing weaknesses in the web server's URL processing, Sofia submits crafted requests to manipulate resource paths. Her tests uncover a severe flaw: the server grants access to restricted system files, exposing sensitive configuration data.\n\nWhich web server attack type is Sofia most likely exploiting?",
        options: [
    ...
    {
        id: 252,
        question: "You are part of the red team assigned to evaluate the physical and social vulnerabilities of a government contractor's office located in a metropolitan business hub. During your pretexting phase, you decide to simulate the role of a third-party IT technician. Upon arrival, the receptionist allows you entry without verification. While moving through the workspace, you casually observe open terminals, unattended printouts, and discarded sticky notes at workstations.\n\nWhich social engineering technique does this scenario best illustrate?",
        options: [
            "Shoulder Surfing",
            "Eavesdropping",
            "Impersonation",
    ...
    {
        id: 253,
        question: "Emma, an ethical hacker at a Chicago-based healthcare provider, is performing a penetration test on the organization's patient record system following a recent data breach. She discovers that attackers gained access to a large volume of encrypted patient records but had no knowledge of the original data or encryption keys. Emma suspects the attackers may have applied a cryptanalytic method that examines encrypted outputs in bulk to detect structural or statistical patterns.\n\nWhich cryptanalysis technique should Emma investigate to assess the system's vulnerability in this scenario?",
        options: [
    ...
    {
        id: 257,
        question: "During an internal red team engagement at Orion Tech Labs, a leading software firm in Austin, Texas, ethical hacker Emily Carter was tasked with evaluating the resilience of the organization's software deployment processes. She repackaged a trusted PDF converter installer with a secondary payload. When an employee executed the installer, the converter installed and functioned normally, but in the background, a hidden executable silently initiated outbound network communication.\n\nWhich technique did Emily most likely use to ensure the malware executed alongside the legitimate application?",
        options: [
            "Downloader",
            "Packer",
            "Dropper",
    ...
    {
        id: 261,
        question: "During a routine software update at Horizon Solutions, a mid-sized IT firm in Raleigh, North Carolina, an employee downloads a free utility from a popular third-party site. During the installation, the user is prompted to install an optional 'productivity toolbar' and a 'system optimization tool.' Shortly after, the employee notices intermittent pop-up ads, an altered browser homepage, and sluggish PC performance.\n\nWhat type of threat are these unwanted programs most likely classified as?",
        options: [
    ...
    {
        id: 263,
        question: "In the bustling financial hub of Charlotte, North Carolina, ethical hacker Raj Patel is contracted by TrustBank, a regional US bank, to evaluate their online loan application portal. By submitting a specially crafted document, he triggers a response that exposes internal server file paths and sensitive configuration data, including database connection strings. The issue arises from the portal's handling of external references in document parsing.\n\nWhich type of vulnerability is Raj most likely exploiting in TrustBank's online loan application portal?",
        options: [
    ...
    {
        id: 266,
        question: "Dr. Evelyn Reed, a cybersecurity expert, was called in to investigate a series of unusual activities at 'Global Innovations Inc.' The first red flag was a surge in spear-phishing emails targeting senior management. Soon after, the company's web server showed unexpected outbound traffic to unfamiliar IP addresses. A network audit revealed that multiple underutilized printers and routers had unauthorized firmware installed. Further review uncovered inconsistencies in file access logs linked to the R&D department, including unusually large data transfers occurring during non-business hours.\n\nWhich phase of the Advanced Persistent Threat (APT) lifecycle is Global Innovations Inc. most likely experiencing?",
        options: [
            "Initial Intrusion",
            "Expansion",
    ...
    {
        id: 267,
        question: "Alice, a software developer, digitally signs an email contract and sends it to Bob. Later, a dispute arises and Alice claims she never sent the agreement. However, Bob produces the email with Alice's unique digital signature, which unequivocally links the message to her.\n\nIn information security terms, what principle is illustrated by Bob's ability to prove Alice's authorship of the email?",
        options: [
            "Confidentiality",
            "Integrity",
            "Non-Repudiation",
    ...
    {
        id: 284,
        question: "In Seattle, Washington, ethical hacker Mia Chen is tasked with testing the network defenses of Pacific Shipping Co., a major logistics firm. During her penetration test, Mia targets the company's external-facing web server, which handles customer tracking requests. She observes that the security system filtering traffic to this server analyzes incoming SSH and DNS requests to block unauthorized access attempts.\n\nWhich security system is Mia attempting to bypass during her penetration test of Pacific Shipping Co.'s web server?",
        options: [
    ...
    {
        id: 292,
        question: "As part of a penetration test for a financial firm's smart headquarters in Denver, Colorado, ethical hacker Jordan Lee begins evaluating the IoT infrastructure responsible for lighting, HVAC, and badge-controlled access. Jordan documents details such as device models, manufacturer names, firmware versions, and supported protocols like Zigbee and BLE.\n\nWhich step of the IoT hacking methodology is being carried out in this phase?",
        options: [

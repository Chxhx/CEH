const questionsBatch2 = [
    ...
    {
        "id": 105,
        "question": "A multinational corporation recently survived a severe Distributed Denial-of-Service (DDoS attack, which caused significant downtime and resulted in substantial financial losses. After implementing enhanced security measures, the company contracted you as a cybersecurity consultant to assess their new infrastructure. During the audit, you discovered that the organization uses both hardware and cloud-based solutions to distribute incoming traffic. The primary purpose of these solutions is to absorb and mitigate the effects of a DDoS attack, ensuring that legitimate requests are not affected during such an event.\n\nWhat type of DDoS mitigation strategy is the company utilizing?",
        "options": [
            "Sinkholing: This technique reroutes traffic to a “sinkhole”, a designated IP address where traffic can be analyzed. However, this doesn’t align with the company’s strategy of distributing incoming traffic.",
            "Black Hole Routing: This approach directs all traffic into a non-existent interface (the “black hole”), which wouldn’t distribute traffic but would drop it, making it inconsistent with the company’s strategy.",
            "Rate Limiting: This technique controls the traffic rate using bandwidth management. It doesn’t distribute the traffic but rather restricts it, making it inconsistent with the company’s strategy.",
            "Load Balancing: This approach distributes network or application traffic across many resources to optimize resource use, minimize latency, and maximize throughput. This is consistent with the company’s strategy of distributing incoming traffic."
        ],
        "correctAnswer": 3,
        "userAnswer": null,
        "bookmarked": false
    ...
    {
        "id": 107,
        "question": "During an internal penetration test, a security analyst assesses a web application that interfaces with a backend Oracle database. Initial attempts using standard SQL injection payloads such as ‘ OR ‘1’=’l and UNION SELECT return no useful output and do not affect application behavior. Suspecting input sanitization and error suppression, the analyst crafts a new payload:\n\n1 AND 1 < (SELECT COUNT(* FROM all_users A, all_users B, all_users C Upon submitting this payload, the page takes significantly longer to respond compared to previous inputs, even though no visible output is returned.\n\nWhat type of SQL injection technique is most likely being used here?",
        "options": [
    ...
    {
        "id": 108,
        "question": "You’ve been hired as a Certified Ethical Hacker (CEH) by a large multinational corporation to investigate the unauthorized access of sensitive data from their web application. You discover that the web application uses a custom authorization scheme involving token-based authentication. The system issues tokens, which expire after 30 minutes. In the server logs, you notice multiple failed login attempts using expired tokens within a 3-minute window, followed by successful access with a valid token. You suspect that an attacker exploited the authorization scheme.\n\nWhat could be the most likely attack scenario?",
        "options": [
    ...
    {
        "id": 110,
        "question": "You are a new member of your company’s IT team, and you've been assigned to understand and implement ethical hacking principles to improve the company’s cybersecurity posture. Your supervisor highlights the importance of following the five phases of ethical hacking.\n\nWhat is the correct order of these phases?",
        "options": [
    ...
    {
        "id": 111,
        "question": "As a certified ethical hacker, you have been engaged to evaluate the security protocols of a smart city project. This cutting-edge venture incorporates an interconnected system featuring intelligent traffic lights, public Wi-Fi points, and advanced water management facilities. Upon evaluating the IoT network’s logs, you stumble upon anomalous traffic patterns, signifying a high volume of data exchange between a particular traffic light and an external IP address. Your further investigation reveals that this traffic light has an inexplicably open port.\n\nGiven the gravity of these revelations, what would be your subsequent course of action?",
        "options": [
            ...
            "Attempt to orchestrate a reverse connection from the traffic light to the external IP, aiming to comprehend the type and purpose of the data being transferred.",
            "Isolate the implicated traffic light from the overarching network for a detailed investigation into its firmware to identify any possible security breaches."
        ],
        "correctAnswer": 3,
        "userAnswer": null,
        "bookmarked": false
    ...
    {
        "id": 113,
        "question": "You have recently joined as a cybersecurity analyst at a multinational corporation. Your role includes regular vulnerability assessments of the company’s wide-ranging IT infrastructure. During one of these assessments, you employ the Nessus scanner. The scanner flags a severe vulnerability marked as CVE-2023-12456. This vulnerability specifically targets the SSH (Secure Shell service running on one of the company’s Linux servers. With a CVSS (Common Vulnerability Scoring System score of 9.0, the vulnerability poses a substantial risk. Most concerning is its ability to allow potential remote code execution.\n\nGiven the high-risk nature of the vulnerability, as depicted by the CVSS score, and its potential impact, what course of action should be immediately prioritized from the following choices?",
        "options": [
            "Commence a manual inspection of the discovered SSH service vulnerability. If it cannot be verified, mark it as a false positive and move on to the next task.",
            "Without delay, apply the patch recommended by the vendor. Subsequently, initiate a system-wide reboot during the next downtime scheduled for system maintenance.",
            "Establish a backup SSH service on an unaffected server and reroute all SSH traffic to this server until the vulnerability on the original server is completely resolved.",
            "Temporarily isolate the affected server from the broader network, perform an in-depth system audit, and then apply patches as required based on the findings."
        ],
        "correctAnswer": 1,
        "userAnswer": null,
        "bookmarked": false
    ...
    {
        "id": 116,
        "question": "A large media-streaming company begins receiving complaints from users that their web application is timing out or failing to load. The security team observes that the web server is overwhelmed with a high number of open HTTP connections, most of which appear to be legitimate but are transmitting data at an abnormally slow rate. These connections are never fully completed, causing the server’s resources to remain occupied and unable to process new incoming client requests. The incident response team rules out bandwidth exhaustion and malformed packet-based attacks and suspects an application-layer DoS technique is responsible.\n\nGiven the scenario, which specific type of attack is most likely being executed on the company’s web application?",
        "options": [
            "The attacker uses a flooding attack on the server with random UDP packets to random ports to consume available bandwidth.",
            ...
            "The attacker floods the server with continuous ICMP Echo Request packets, overwhelming its network stack."
        ],
        "correctAnswer": 2,
        "userAnswer": null,
        "bookmarked": false
    ...
    {
        "id": 118,
        "question": "You are a security consultant who has been hired to conduct security awareness training at a mid-sized organization. During the session on social engineering, you emphasize the importance of being vigilant against different types of social engineering attacks.\n\nWhich of the following scenarios best describes a tailgating social engineering attack?",
        "options": [
            "An email is sent to a company's employees claiming there is an urgent system update and they need to enter their login credentials to complete the process.",
            "An attacker calls a company’s customer service representative, claiming they lost their account details and need assistance in recovering them.",
            "An attacker leaves a USB labelled “Employee Bonus List” in the restroom, hoping someone will insert it into their computer.",
            "A person gains access to the building by following an employee through a secure door"
        ],
        "correctAnswer": 3,
        "userAnswer": null,
        "bookmarked": false
    },
    {
        ...
        "options": [
            ...
            "Conduct real-time monitoring of the server, scrutinize the logs for unusual patterns, and identify the nature of the activities to devise an immediate countermeasure.",
            "Conduct a thorough audit of all outbound traffic and scrutinize the destination IPs to gain insights into the attacker’s network."
        ],
        "correctAnswer": 2,
        "userAnswer": null,
        "bookmarked": false
    ...
    {
        "id": 120,
        "question": "A critical infrastructure facility has an extensive network of IoT devices integrated with its OT systems. Following a recent surge in cyberattacks on similar facilities globally, the security team received an anonymous email detailing a possible attack on their systems. The attacker allegedly plans to compromise the IoT devices and use them as a gateway to the OT systems.\n\nWhat should be the immediate action of the security team to mitigate this threat?",
        "options": [
    ...
    {
        "id": 122,
        "question": "In your role as a cybersecurity specialist for a growing tech company, you are tasked with performing a regular network scan to check for any potential vulnerabilities. Your manager has specifically asked you to use a NULL scan for this round.\n\nIn the context of TCP NULL scanning, what response (or lack thereof would indicate that a particular port on a target machine is closed?",
        "options": [
    ...
    {
        "id": 125,
        "question": "A penetration tester is hired by a company to assess its vulnerability to social engineering attacks targeting its IT department. The tester decides to use a sophisticated pretext involving technical jargon and insider information to deceive employees into revealing their network credentials.\n\nWhat is the most effective social engineering technique the tester should employ to maximize the chances of obtaining valid credentials without raising suspicion?",
        "options": [
    ...
    },
    {
        ...
        "options": [
            ...
            "Modify the URL parameter to userlD=l OR 1=1 and observe if all orders are displayed",
    ...
    {
        "id": 144,
        "question": "A penetration tester is tasked with enumerating user accounts and network resources in a highly secured Windows environment where standard methods like SMB null sessions are blocked. The network employs strict firewall rules and intrusion detection systems to prevent unauthorized access.\n\nWhich technique should the tester use to discreetly gather the required information without triggering security alarms?",
        "options": [
    ...
    {
        "id": 160,
        "question": "A hacker is analyzing a system that uses two rounds of symmetric encryption with different keys. To speed up key recovery, the attacker encrypts the known plaintext with all possible values of the first key and stores the intermediate ciphertexts. Then, they decrypt the final ciphertext using all possible values of the second key and compare the results to the stored values.\n\nWhich cryptanalytic method does this approach represent?",
        "options": [
    ...
    {
        "id": 177,
        "question": "A red team operator wants to obtain credentials from a Windows machine without touching the LSASS process memory, as security controls and Credential Guard are active. Instead, they leverage a method that uses the Security Support Provider Interface (SSPI to generate NetNTLM responses within the context of the logged-in user and collect those responses for offline cracking.\n\nWhich attack technique is being used in this scenario?",
        "options": [
